...
require_sudo() { [[ "${EUID}" -eq 0 ]] || die "Run as root: curl ... | sudo bash"; }
...
install_firacode_nerd_font() {
  ...
  as_user "rm -rf '${TMP}'" || true
  as_user "command -v fc-cache >/dev/null 2>&1 && fc-cache -f '${TARGET_HOME}/.local/share/fonts' >/dev/null 2>&1 || true"
}
